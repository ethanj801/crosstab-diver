...
def preference(row):
    ...
    if early == "5":
        return (
            "no_presidential_vote_reported_at_interview",
            "reported_no_presidential_vote_at_interview",
            "CC20_364a",
        )
...
def load_pool(source, state):
    ...
    with source.open(newline="", encoding="utf-8-sig") as stream:
        ...
        for row in reader:
            ...
            if row["cit1"] != "1":
                exclusions[
                    "noncitizen" if row["cit1"] == "2" else "citizenship_unknown"
                ] += 1
                continue
            ...
            if not math.isfinite(weight) or weight <= 0:
                raise ValueError(
                    "Citizen adult has a nonpositive or nonfinite commonweight"
                )
            ...
            if race == "3" or hispanic == "1":
                ...
                continue
            if row["gender"] not in {"1", "2"} or row["educ"] not in {
                "1",
                "2",
                "3",
                "4",
            ...
            pool.append(
                {
                    "source_caseid": row["caseid"],
                    "state_fips": f"{STATES[state][0]:02d}",
                    "citizen": True,
                    "age_2020": age,
                    "age_group": "18-29"
                    if age < 30
                    ...
                    "presidential_preference": vote,
                    "presidential_response_kind": vote_kind,
...
def main():
    ...
    parser.add_argument(
        "--input",
        type=Path,
        default=Path.home() / "Downloads/dataverse_files/CES20_Common_OUTPUT_vv.csv",
    )
    ...
    parser.add_argument(
        "--output", type=Path, help="default: data/<state>-2020/sample_<size>.json"
    )
    ...
    try:
        ...
        sample = [
            {
                "id": f"{args.state.lower()}2020-{i + 1:06d}",
                **row,
                "draw_probability": row["ces_commonweight"] / total_weight,
                "base_weight": 1.0,
            }
        ...
        result = {
            ...
            "seed": args.seed,
            "state": args.state,
            "state_fips": f"{STATES[args.state][0]:02d}",
            ...
            "source_sha256": hashlib.file_digest(
                args.input.open("rb"), "sha256"
            ).hexdigest(),
            "source_dataset_doi": "10.7910/DVN/E9N6PH",
            "source_weight": "commonweight",
            ...
            "duplicate_draws": len(sample)
            - len({row["source_caseid"] for row in sample}),
            ...
            "source_weighted_margins": margins(pool, True),
            "sample_unweighted_margins": margins(sample, False),
            "respondents": sample,
        }
        output = (
            args.output
            ...
            / f"sample_{args.size}.json"
        )
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(json.dumps(result, indent=2, allow_nan=False) + "\n")
        print(
            f"Wrote {len(sample)} draws ({result['unique_source_respondents']} unique CES respondents) from {len(pool)} source respondents to {output}"
        )
