...
class RakingTests(unittest.TestCase):
    ...
    def test_actual_samples_match_targets_and_preserve_outcomes(self):
        for state in ("georgia",):
            with self.subTest(state=state):
                ...
                self.assertTrue(
                    all(math.isfinite(r["weight"]) and r["weight"] > 0 for r in records)
                )
                for dim, cats in targets["margins"].items():
                    for cat, target in cats.items():
                        actual = (
                            sum(r["weight"] for r in records if r[dim] == cat) / total
                        )
                ...
                self.assertEqual(
                    [r["weight"] for r in prepare(sample, targets)["respondents"]],
    ...
    def test_preparation_rejects_mixed_states(self):
        ...
        targets = json.loads(
            (DATA / "georgia-2020/raking_targets_cps_nov2020.json").read_text()
        )
