...
DATA = Path(__file__).resolve().parents[1] / "data" / "california-2020"
...
class RakingTests(unittest.TestCase):
    ...
    def test_actual_sample_matches_every_target_and_preserves_outcomes(self):
        sample = json.loads((DATA / "sample_600.json").read_text())
        targets = json.loads((DATA / "raking_targets_cps_nov2020.json").read_text())
        demo = prepare(sample, targets)
        records = demo["respondents"]
        self.assertEqual(len(records), 600)
        self.assertEqual(sum(r["preference"] == "D" for r in records), 401)
        self.assertEqual(len({r["id"] for r in records}), 600)
        ...
        # Preference changes cannot affect demographic weights.
